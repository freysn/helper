#ifndef __BZIP_HELPER__
#define __BZIP_HELPER__
...
template<typename T>
std::pair<size_t,int> bzip_compress(FILE *f, T* buf, size_t nElems)
{
  int bzError;
  BZFILE *bzf;
  ...
  {
    const size_t chunkElems = (1024*1024*1024)/sizeof(T);
    assert(chunkElems>=1);
...
template<typename T>
int bzip_compress(std::string fname, std::vector<T>& buf)
{
  return bzip_compress(fname, &buf[0], buf.size());
}

template<typename T>
int bzip_decompress(std::vector<T>& out, FILE *f)
{
  int bzError;
  BZFILE *bzf;
...
template<typename T>
int bzip_decompress(std::vector<T>& out, std::string fname)
{
  FILE* f;
